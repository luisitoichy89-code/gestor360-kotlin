...
class InventarioRepository(private val context: Context = AppContextHolder.context) {
    ...
    suspend fun contarColaPendiente(androidId: String): Result<Int> {
        ...
        return try {
            ...
            Log.e("InventarioRepo", "Error contando cola", e)
            Result.failure(e)
        }
    }
    ...
    suspend fun contarVentasSinTurno(): Result<Int> {
        val localId = localIdActivo()
        return try {
            val huerfanas = db.ventaDao().obtenerTodas(localId).count { it.turnoId == null }
            Result.success(huerfanas)
        } catch (e: Exception) {
            Log.e("InventarioRepo", "Error contando ventas sin turno", e)
            Result.failure(e)
        }
    }
    ...
    suspend fun contarDevolucionesPendientes(): Result<Int> {
        val localId = localIdActivo()
        return try {
            val cache = db.devolucionCacheDao().obtener(localId)
            val pendientes = cache?.toModel()?.count {
                it.estado != "aprobada_stock" && it.estado != "aprobada_merma" && it.estado != "rechazada"
            } ?: 0
            Result.success(pendientes)
        } catch (e: Exception) {
            Log.e("InventarioRepo", "Error contando devoluciones", e)
            Result.failure(e)
        }
    }
    ...
    suspend fun contarMermasPendientes(): Result<Int> {
        val localId = localIdActivo()
        return try {
            val pendientes = db.mermaDao().obtenerPendientes(localId).size
            Result.success(pendientes)
        } catch (e: Exception) {
            Log.e("InventarioRepo", "Error contando mermas", e)
            Result.failure(e)
        }
    }
    ...
    suspend fun haySolicitudesPendientes(): Result<Int> {
        val localId = localIdActivo()
        return try {
            val cache = db.aprobacionStockCacheDao().obtener(localId)
            val pendientes = cache?.toModel()?.count { it.estado == "pendiente" } ?: 0
            Result.success(pendientes)
        } catch (e: Exception) {
            Log.e("InventarioRepo", "Error contando aprobaciones", e)
            Result.failure(e)
        }
    }
}
