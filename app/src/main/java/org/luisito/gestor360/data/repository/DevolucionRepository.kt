...
class DevolucionRepository(private val context: Context = AppContextHolder.context) {
    ...
    suspend fun solicitar(androidId: String, productoId: Long, productoNombre: String, cantidad: Double, metodo: String, motivo: String): Result<Unit> {
        ...
        val nueva = Devolucion(id = idTemporal, producto_id = productoId, producto_nombre = productoNombre, cantidad = cantidad, metodo = metodo, motivo = motivo, estado = "pendiente", local_id = localId)
        ...
        try { SyncReporter.reportar(androidId, localId, "solicitar_devolucion", payload) } catch (_: Exception) {}
    ...
    suspend fun resolver(androidId: String, id: Long, estado: String, destino: String? = null): Result<Unit> {
        if (!NetworkMonitor.hayInternet(context)) return Result.failure(IllegalStateException("Necesitas conexión para resolver una devolución"))
        val localId = localIdActivo()
        return try {
